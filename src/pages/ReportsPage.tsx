...
export const ReportsPage: React.FC = () => {
  ...
  const handleConfirmCloseRegister = async () => {
    try {
      ...
      await loadReports();
      alert('¡Caja del día cerrada exitosamente!');
    } catch (err) {
  ...
  const handleStartNewDay = async () => {
    if (window.confirm('¿Deseas iniciar un nuevo día operativo? Las ventas del día volverán a 0.00 para el nuevo turno.')) {
      try {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl border border-gray-200 shadow-xs gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-gray-800">Panel de Ventas y Caja</h1>
        ...
        <div className="flex flex-wrap gap-2.5">
          ...
          {/* Botón Cierre de Caja / Nuevo Día */}
          {data.isRegisterClosed ? (
            <button
              type="button"
          ...
          >
            <span>📑</span>
            <span>Descargo Oficial Imprimible</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Ventas de Hoy</span>
          <p className="text-3xl font-black text-gray-900 mt-2">
            {settings.currency} {data.todayTotal.toFixed(2)}
          </p>
          <span className="text-[11px] text-emerald-600 font-semibold mt-2">
            {data.todayOrdersList.length} comandas pagadas
          </span>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Esta Semana</span>
          <p className="text-3xl font-black text-gray-900 mt-2">
            {settings.currency} {data.weekTotal.toFixed(2)}
          </p>
          <span className="text-[11px] text-blue-600 font-semibold mt-2">● Facturación lunes a domingo</span>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Este Mes</span>
          <p className="text-3xl font-black text-gray-900 mt-2">
            {settings.currency} {data.monthTotal.toFixed(2)}
          </p>
          <span className="text-[11px] text-purple-600 font-semibold mt-2">● Mes en curso</span>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Venta Anual</span>
          <p className="text-3xl font-black text-gray-900 mt-2">
            {settings.currency} {data.yearTotal.toFixed(2)}
          </p>
          <span className="text-[11px] text-amber-600 font-semibold mt-2">● Acumulado del año</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase">Comandas Totales Atendidas</p>
            <h3 className="text-2xl font-black text-gray-800 mt-1">{data.totalPaidOrders} cuentas</h3>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase">Ticket Promedio por Mesa</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div key={idx} className="flex flex-col items-center gap-2 h-full justify-end group">
                  ...
                  <div
                    ...
                    className={`w-full max-w-[38px] rounded-t-lg transition-all ${
                      day.total > 0 ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-100'
                    }`}
                  />
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-gray-200 shadow-xs space-y-4">
          <h2 className="text-sm font-bold text-gray-800 uppercase tracking-wider">
            Top 5 Platos con Mayor Rotación
          </h2>
          {data.topProducts.length === 0 ? (
            <p className="text-xs text-gray-400 py-6 text-center italic">
              No hay suficientes registros de ventas aún.
            </p>
          ) : (
                ...
                <div key={idx} className="flex items-center justify-between border-b border-gray-100 pb-2.5 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">{prod.quantity} unidades vendidas</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white text-gray-900 w-full max-w-lg rounded-2xl p-6 shadow-2xl space-y-4 border border-gray-200">
            <div className="flex justify-between items-center border-b pb-3">
              <div>
                <h3 className="text-lg font-bold text-gray-800">Cierre de Caja del Día (Cierre Z)</h3>
                <p className="text-xs text-gray-500">Conciliación de ingresos y conteo de efectivo físico</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-200 text-xs">
              <div className="flex justify-between">
                <span className="text-gray-500 font-semibold">Total Ventas Facturadas Hoy:</span>
                <strong className="text-gray-900">{settings.currency} {data.todayTotal.toFixed(2)}</strong>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500 font-semibold">Fondo Inicial de Caja:</span>
                <input
                  type="number"
                  step="0.1"
                  value={initialCash}
              ...
              <div className="border-t border-gray-200 pt-2 flex justify-between font-bold text-sm">
                <span>Efectivo Total Teórico Esperado:</span>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-xs pt-1">
                <span className="font-semibold text-gray-500">Diferencia (Sobrante/Faltante):</span>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Ej. Cuadre exacto / Billetes desgastados guardados..."
                value={cashNotes}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white text-gray-900 w-full max-w-2xl rounded-2xl p-8 shadow-2xl space-y-6 border border-gray-200 max-h-[90vh] overflow-y-auto print:max-w-full print:shadow-none print:border-0 print:p-0">
            ...
            <div className="flex gap-1.5 p-1 bg-gray-100 rounded-xl print:hidden">
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedPeriod === 'DAILY' ? 'bg-white text-red-600 shadow-xs' : 'text-gray-500 hover:text-gray-800'
                }`}
              >
                ☀️ Diario
              </button>
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedPeriod === 'WEEKLY' ? 'bg-white text-red-600 shadow-xs' : 'text-gray-500 hover:text-gray-800'
                }`}
              >
                📅 Semanal
              </button>
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedPeriod === 'MONTHLY' ? 'bg-white text-red-600 shadow-xs' : 'text-gray-500 hover:text-gray-800'
                }`}
              >
                🗓️ Mensual
              </button>
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedPeriod === 'ANNUAL' ? 'bg-white text-red-600 shadow-xs' : 'text-gray-500 hover:text-gray-800'
                }`}
              >
                📈 Anual
              </button>
            ...
            <div className="text-center border-b pb-4 border-gray-200">
              <h2 className="text-xl font-black uppercase tracking-wider">{settings.name}</h2>
              ...
              <h3 className="text-sm font-bold text-red-600 mt-2 uppercase tracking-wide">
                DOCUMENTO OFICIAL DE DESCARGO DE VENTAS ({selectedPeriod})
              </h3>
            ...
            <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
              <div>
                <span className="text-[11px] font-bold text-gray-500 uppercase">Facturación del Período:</span>
              ...
              <div>
                <span className="text-[11px] font-bold text-gray-500 uppercase">Responsable / Emisor:</span>
            ...
            <div>
              <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">
                {selectedPeriod === 'DAILY' && 'Detalle de Comandas Facturadas en el Turno'}
                {selectedPeriod === 'WEEKLY' && 'Desglose Diario de la Semana'}
                {selectedPeriod === 'MONTHLY' && 'Desglose Semanal del Mes en Curso'}
                ...
                <tbody className="divide-y divide-gray-200">
                  {selectedPeriod === 'DAILY' &&
                    (data.todayOrdersList.length === 0 ? (
                      ...
                      data.todayOrdersList.map((ord, i) => (
                        <tr key={i}>
                          <td className="p-2.5 font-semibold">Mesa #{ord.tableNumber}</td>
                          <td className="p-2.5 text-center text-gray-500">Hora: {ord.time}</td>
                          <td className="p-2.5 text-right font-bold">{settings.currency} {ord.total.toFixed(2)}</td>
                        </tr>
                      ))
                    ))}
                  ...
                  {selectedPeriod === 'WEEKLY' &&
                    data.last7Days.map((d, i) => (
                      <tr key={i}>
                        <td className="p-2.5 font-semibold">{translateDay(d.dayName)} ({d.date})</td>
                        <td className="p-2.5 text-center text-gray-500">{d.orderCount} cuentas</td>
                        <td className="p-2.5 text-right font-bold">{settings.currency} {d.total.toFixed(2)}</td>
                      </tr>
                    ))}
                  ...
                  {selectedPeriod === 'MONTHLY' &&
                    data.monthWeeks.map((w, i) => (
                      <tr key={i}>
                        ...
                        <td className="p-2.5 text-center text-gray-500">{w.orderCount} cuentas</td>
                        <td className="p-2.5 text-right font-bold">{settings.currency} {w.total.toFixed(2)}</td>
                      </tr>
                    ))}
                  ...
                  {selectedPeriod === 'ANNUAL' &&
                    data.yearMonths.map((m, i) => (
                      <tr key={i}>
                        ...
                        <td className="p-2.5 text-center text-gray-500">{m.orderCount} cuentas</td>
                        <td className="p-2.5 text-right font-bold">{settings.currency} {m.total.toFixed(2)}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-2 gap-8 pt-10 text-center">
              <div className="border-t border-gray-400 pt-2">
                <p className="text-xs font-bold text-gray-700">Responsable de Caja / POS</p>
                <p className="text-[10px] text-gray-400">Firma y Sello</p>
              </div>
              <div className="border-t border-gray-400 pt-2">
                <p className="text-xs font-bold text-gray-700">Administración General</p>
            ...
            <div className="flex gap-2 pt-4 border-t border-gray-100 print:hidden">
              ...
              >
                🖨️ Imprimir Descargo ({selectedPeriod})
              </button>
              <button
                type="button"
